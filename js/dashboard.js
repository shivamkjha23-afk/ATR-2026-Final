...
function addImageChunk(doc, sourceCanvas, opts) {
  const {
    chunkPxTop,
    chunkPxHeight,
    x,
  ...
  sliceCanvas.height = chunkPxHeight;
...
function addPaginatedElementImage(doc, sourceCanvas, opts) {
  const {
    x,
    y,
    maxWidth,
  ...
  if (scaledTotalHeightMm <= maxHeight) {
    addImageChunk(doc, sourceCanvas, {
      chunkPxTop: 0,
  ...
  try {
    ...
    for (let idx = 0; idx < sections.length; idx += 1) {
      ...
      if (tableElement) {
        ...
        addPaginatedElementImage(doc, tableCanvas, {
          x: contentX,
          y: contentTopY,
      ...
      if (chartCanvas) {
        addDashboardPdfPage(doc, reportTitle, pageWidth, pageHeight, dateLabel);
        addPaginatedElementImage(doc, chartCanvas, {
          x: contentX,
          y: contentTopY,
          maxWidth: contentWidth,
          maxHeight: contentHeight,
